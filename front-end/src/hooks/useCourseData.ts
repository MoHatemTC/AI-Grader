      ...
      try {
        const sql = `
           SELECT 
              ...
              at.title,
              at.description,
              (SELECT COUNT(*) FROM assignment_submissions WHERE assignment_id = a.id AND status != 'not_submitted') AS totalSubmissions,
              (SELECT COUNT(*) FROM assignment_submissions WHERE assignment_id = a.id AND grade IS NOT NULL) AS scoredSubmissions,
              (SELECT COUNT(*) FROM assignment_submissions WHERE assignment_id = a.id AND status = 'not_submitted') AS pendingSubmissions,
              a.deadline AS dueDate,
              (SELECT COUNT(DISTINCT student_id) FROM assignment_submissions WHERE assignment_id = a.id AND status != 'not_submitted' ) AS uniqueSubmissions
            FROM assignments a
            LEFT JOIN (
        ...
        return result.data.rows.map(
          (row: any) =>
            ({
              ...
              totalSubmissions: row[4] || 0,
              scoredSubmissions: row[5] || 0,
              pendingSubmissions: row[6] || 0,
              dueDate: row[7]
                ? new Date(row[7]).toISOString().split("T")[0]
                : null,
              uniqueSubmissions: row[8] || 0,
