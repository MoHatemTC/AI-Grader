...
from AIGrader_2 import AIGrader
...
class GraderOutput(BaseModel):
    user_id: Optional[int] = None  # Optional user ID for bulk grading
    scope_score: int
    scope_comment: str
    quality_score: int
    quality_comment: str

@app.post("/evaluate", response_model=List[GraderOutput])
        ...
        if grader_input.users:
            for user in grader_input.users:
                ...
                scope, quality = grader.process_tasks(
                    ...
                    grader_input.scope_rubric,
                    grader_input.requirements_rubric,
                ...
                results.append(
                    {
                        ...
                        "scope_comment": scope["scope_comment"],
...
async def generate_rubric(
    ...
    try:
        ...
        raise HTTPException(
            status_code=400, detail=f"Rubric generation error: {str(3)}"
        )
...
async def save_rubric(request: RubricSaveRequest, supabase: Client = Depends(get_db)):
    try:
        data = {
            ...
            "passed_score": request.passed_score,
...
class UserGradeRequest(BaseModel):
    scope_score: int
    scope_comment: str = ""
    quality_score: int
    quality_comment: str = ""
...
async def save_grading_results(
    request: UserGradeRequest, supabase: Client = Depends(get_db)
):
    try:
        # Validate scores
        if not (0 <= request.scope_score <= 100) or not (
            0 <= request.quality_score <= 100
        ):
            raise HTTPException(
        ...
        grading_data = {
            ...
            "scope_score": request.scope_score,
            "scope_comment": request.scope_comment,
